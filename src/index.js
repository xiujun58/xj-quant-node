...
function hasSenderRestrictions () {
    return Boolean(
        config.telegramAllowedBotId ||
        config.telegramAllowedBotUsername ||
...
function isAllowedSender (message) {
    ...
    const allowChatId = config.telegramAllowedChatId;

    if (!hasSenderRestrictions()) {
        return { pass: true, reason: 'no_restriction' };
    }

    if (allowChatId && String(chat.id) !== String(allowChatId)) {
        return { pass: false, reason: 'chat_not_allowed' };
    }

    if (!allowId && !allowUsername) {
        return { pass: true, reason: 'chat_only_restriction_pass' };
    }

    if (!from.is_bot) {
        return { pass: false, reason: 'sender_not_bot' };
    }

    if (allowId && String(from.id) !== String(allowId)) {
        return { pass: false, reason: 'bot_id_not_match' };
    }

    if (allowUsername) {
        const senderUsername = String(from.username || '').toLowerCase();
        if (senderUsername !== allowUsername) {
            return { pass: false, reason: 'bot_username_not_match' };
        }
    }

    return { pass: true, reason: 'sender_allowed' };
}
...
async function runBot (services) {
    ...
    if (!hasSenderRestrictions()) {
        throw new Error(
            'bot模式要求设置 TELEGRAM_ALLOWED_BOT_ID / TELEGRAM_ALLOWED_BOT_USERNAME / TELEGRAM_ALLOWED_CHAT_ID 中至少一个'
        );
    }
    ...
    while (true) {
        try {
            ...
            for (const update of updates) {
                ...
                // const senderCheck = isAllowedSender(message);

                // if (!senderCheck.pass) {
                //   process.stdout.write(`[BOT] 忽略消息: ${senderCheck.reason}\n`);
                //   continue;
                // }
